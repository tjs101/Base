...
@property(nonatomic, assign) BOOL wasTriggeredByCode;
...
- (void)triggerPullToRefresh {
    self.pullToRefreshView.wasTriggeredByCode = YES;
    self.pullToRefreshView.state = SVPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)removePullRefreshActionHandler
{
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"Pull down to refresh...",),
...
- (void)layoutSubviews {
    ...
    if (self.backgroundImage) {
        ...
        CGRect rect = CGRectMake(0, self.frame.size.height-self.backgroundImage.frame.size.height, self.backgroundImage.frame.size.width, self.backgroundImage.frame.size.height);
    ...
    if(hasCustomView) {
        ...
        CGSize titleSize = HC_MULTILINE_TEXTSIZE(self.titleLabel.text, self.titleLabel.font, CGSizeMake(labelMaxWidth,self.titleLabel.font.lineHeight), self.titleLabel.lineBreakMode);
        
        CGSize subtitleSize = HC_MULTILINE_TEXTSIZE(self.subtitleLabel.text, self.subtitleLabel.font, CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight), self.subtitleLabel.lineBreakMode);
...
- (void)showBackgroundImage:(NSString *)imageName
{
    if (nil==self.backgroundImage) {
        self.backgroundImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
